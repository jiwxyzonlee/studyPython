...
#헤드라인,정치, 경제, 사회, 생활문화, 세계, IT과학
    ...
    pol = bs_obj.find("div", {"id":"section_politics"})
    mlist2_pol = bs_obj.findAll("ul", {"class":"mlist2 no_bg"})
    span_pol = mlist2_pol.findAll("span", {"class":"writing"})
    #print(mlist2_pol)
    print(span_pol)
...
#정치, 경제, 사회, 생활문화, 세계, IT과학
    for li2 in mlist2_pol:
        strong = li2.find("strong")
        #print(strong)
        strong = strong.text
        print(strong)
        #print(strong)
        span = li2.find("span").text
        print(span)
